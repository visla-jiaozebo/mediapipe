...
cc_library(
    name = "libopencv_arm64-v8a",
    srcs = glob(
        [
            paths.join(PREFIX, "libs/libopencv_core.so"),
            paths.join(PREFIX, "libs/libopencv_imgproc.so"),
        ],
    ),
    hdrs = glob([paths.join(PREFIX, "jni/include/opencv2/**/*.h*")]),
    includes = [paths.join(PREFIX, "jni/include/")],
    linkstatic = 1,
