...
int main(int argc, char** argv) {
  ...
  while (av_read_frame(fmt_ctx, pkt) >= 0) {
    ...
    if (avcodec_send_packet(codec_ctx, pkt) == 0) {
      ...
      if (r == 0) {
        ...
        rgb_buffer.resize(num_bytes);
        av_image_fill_arrays(rgb_frame->data, rgb_frame->linesize, rgb_buffer.data(), AV_PIX_FMT_RGB24, frame->width, frame->height, 1);
        sws_scale(sws, frame->data, frame->linesize, 0, frame->height, rgb_frame->data, rgb_frame->linesize);
        got = 1;
  ...
  auto status = api.Initialize(graph_path.c_str());
  if (!status.ok()) {
  ...
  view.data = rgb_buffer.data();
  view.width = target_w;
  view.height = target_h;
  view.stride = target_w * 3;
  view.format = mediapipe::FaceLmPixelFormat::kRGB;
