...
try:
    ...
    # Display the image
    ax.imshow(img, extent=[0, 1, 1, 0])  # extent to normalize coordinates
    ...
    # Updated coordinate points from your data
    # First group: 84, 85, 86, 87, 88, 89, 90, 96, 97, 98, 99, 100
    upper_lip_points = {
        84: (0.426186, 0.593516),  # Index 168, 169
        85: (0.453348, 0.586128),  # Index 170, 171
        86: (0.481258, 0.582594),  # Index 172, 173
        87: (0.500000, 0.584476),  # Index 174, 175
        88: (0.518742, 0.582594),  # Index 176, 177
        89: (0.546652, 0.586128),  # Index 178, 179
        90: (0.573814, 0.593516),  # Index 180, 181
        96: (0.443456, 0.620391),  # Index 192, 193
        97: (0.433718, 0.595595),  # Index 194, 195
        98: (0.466898, 0.597025),  # Index 196, 197
        99: (0.500000, 0.599883),  # Index 198, 199
        100: (0.533102, 0.597025), # Index 200, 201
    }
    
    # Second group: 90, 91, 92, 93, 94, 95, 96, 100, 101, 102, 103, 84
    lower_lip_points = {
        90: (0.573814, 0.593516),  # Index 180, 181
        91: (0.556544, 0.620391),  # Index 182, 183
        92: (0.531320, 0.639672),  # Index 184, 185
        93: (0.500000, 0.644911),  # Index 186, 187
        94: (0.468680, 0.639672),  # Index 188, 189
        95: (0.443456, 0.620391),  # Index 190, 191
        96: (0.443456, 0.620391),  # Index 192, 193 (duplicate)
        100: (0.533102, 0.597025), # Index 200, 201
        101: (0.566282, 0.595595), # Index 202, 203
        102: (0.534634, 0.610720), # Index 204, 205
        103: (0.500000, 0.616173), # Index 206, 207
        84: (0.426186, 0.593516),  # Index 168, 169 (duplicate)
    }

    upper_lip_coordinates_idx = [84, 85, 96, 96, 85, 97, 97, 85, 86, 86, 97, 98, 86, 98, 87, 87, 98, 88,
      88, 98, 99, 88, 99, 89, 89, 99, 100, 89, 100, 90,]
    lower_lip_coordinates_idx = [90, 100, 91, 100, 91, 101, 101, 91, 92, 101, 92, 102, 102, 92, 93, 102,
    ...
    # for idx, (x, y) in upper_lip_points.items():
    #     ax.plot(x, y, 'ro', markersize=8, markeredgecolor='white', markeredgewidth=1)
    #     ax.annotate(f'{idx}', (x, y), xytext=(3, 3), textcoords='offset points', 
    #                fontsize=8, color='white', weight='bold')
    
    # for idx, (x, y) in lower_lip_points.items():
    #     ax.plot(x, y, 'bo', markersize=8, markeredgecolor='white', markeredgewidth=1)
    #     ax.annotate(f'{idx}', (x, y), xytext=(3, 3), textcoords='offset points', 
    #                fontsize=8, color='white', weight='bold')
    if False:
      for idx in upper_lip_coordinates_idx:
          x, y = (coordinates_values[2*idx]* 1280 - 502.5)/262.5, (coordinates_values[2*idx+1] * 1280 - 710)/167.5
          print(f'{idx}:{x},{y}')
          ax.plot(x, y , 'ro', markersize=4, markeredgecolor='green', markeredgewidth=1)
          ax.annotate(f'{idx}', (x, y), xytext=(2, 2), textcoords='offset points',
    ...
    ax.set_title('Lip Key Points Overlaid on mouth.png')
    
    plt.tight_layout()
    plt.savefig('mouth_with_keypoints.png', dpi=300, bbox_inches='tight')
    plt.show()
