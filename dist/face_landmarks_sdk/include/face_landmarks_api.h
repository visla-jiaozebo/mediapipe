...
enum class FaceLmPixelFormat {
  kRGB,    // 3 channels, interleaved
  kBGR,    // 3 channels, interleaved
  kRGBA,   // 4 channels, interleaved
  kBGRA,   // 4 channels, interleaved
  kGRAY,   // 1 channel
};

// Simple view of an image buffer supplied by the caller. Data must remain
// valid until ProcessFrame returns. Stride is bytes between successive rows.
struct FaceLmImageView {
  const uint8_t* data = nullptr;
  int width = 0;
  int height = 0;
  int stride = 0;  // bytes per row; if 0, treated as tightly packed.
  FaceLmPixelFormat format = FaceLmPixelFormat::kRGB;
};
...
class FACE_LM_EXPORT FaceLandmarksApi {
 public:
  ...
  FaceLmStatus Initialize(const std::string& graph_path =
                              "mediapipe/graphs/face_landmarks/face_landmarks_desktop_live.pbtxt");
